...
def bid_calculation(weights_new, machinenumber, processing_time,
                    current, total, due_date_operation, total_rp, due_date, now, job_priority, queue_length):
    attribute = [0] * noAttributes

    # attribute[0] = processing_time / 8.75 * weights_new[machinenumber - 1][0]
    attribute[0] = (current - 1) / (5 - 1) * weights_new[machinenumber - 1][0]
    attribute[1] = (due_date - now + 400) / (97.5 + 400) * \
                   weights_new[machinenumber - 1][1]
    attribute[2] = total_rp / 21.25 * weights_new[machinenumber - 1][2]
    attribute[3] = (((due_date - now) / total_rp) + 7) / (12 + 7) * weights_new[machinenumber - 1][3]  # Critical Ratio
    attribute[4] = (job_priority - 1) / (10 - 1) * weights_new[machinenumber - 1][4]  # Job Weight
    attribute[5] = queue_length / 25 * weights_new[machinenumber - 1][5]  # Queue length
...
def next_workstation(job, job_shop, env, all_store):
    if job.currentOperation + 1 <= job.numberOfOperations:
        ...
        if (job_shop.WIP > 2500) | (env.now > 100_000):
            # print(job_shop.WIP)
            job_shop.end_event.succeed()
...
def choose_job_queue(weights_new_job, machinenumber, processing_time, due_date, env,
    ...
    # print((due_date - processing_time - setup_time - env.now - (-500)) / (100 + 500))
...
def run_linear(filename1, filename2, filename3, arrival_time_mean, due_date_k, alpha):
    ...
    population_size = totalAttributes + 5
...
if __name__ == '__main__':
    ...
    skip_bid = [7, 0, 1, 2, 3, 4, 5, 7, 7, 7]
    skip_seq = [3, 3, 3, 3, 3, 3, 3, 0, 1, 2]
    for skip in range(1, 2):
        print("Current run is:" + str(utilization[3]) + "-" + str(due_date_settings[3]) + "-" + str(
            learning_decay_rate[5]) + "-" + str(skip_bid[skip]) + "-" + str(skip_seq[skip]))
        str1 = "Runs/Attribute_Runs/Run-NoSetup1-" + str(utilization[3]) + "-" + str(
            due_date_settings[3]) + "-" + str(str(learning_decay_rate[5])) + "-" + str(skip_bid[skip]) + "-" + str(
            skip_seq[skip]) + ".txt"
        str2 = "Runs/Attribute_Runs/Run-weights-NoSetup1-" + str(utilization[3]) + "-" + str(
            due_date_settings[3]) + "-" + str(str(learning_decay_rate[5])) + "-" + str(skip_bid[skip]) + "-" + str(
            skip_seq[skip]) + ".csv"
        str3 = "Runs/Attribute_Runs/Run-std-NoSetup1-" + str(utilization[3]) + "-" + str(
            due_date_settings[3]) + "-" + str(str(learning_decay_rate[5])) + "-" + str(skip_bid[skip]) + "-" + str(
            skip_seq[skip]) + ".csv"
        run_linear(str1, str2, str3, arrival_time[3], due_date_settings[3], learning_decay_rate[5])
