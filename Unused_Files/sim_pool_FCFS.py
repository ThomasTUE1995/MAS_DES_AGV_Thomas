...
def bid_calculation(weights_new, machinenumber, pool, processing_time, setup_time, queue, proc_time,
    ...
    attribute[4] = normalize((due_date_operation - (processing_time + setup_time) - proc_time), 60, -200) * \
...
def choose_job_queue(weights_new, machinenumber, processing_time, current, total, due_date, total_rp, env, setup_time, weight):
    attribute = [0] * noAttributesJob
    # attribute[0] = normalize(processing_time, 8.75, 0) * weights_new[machinenumber - 1][noAttributes]
    attribute[0] = normalize((total - current + 1), total, 1) * weights_new[machinenumber - 1][noAttributes]
    attribute[1] = normalize(due_date - processing_time - env.now, 60, -200) * weights_new[machinenumber - 1][noAttributes + 1]
    attribute[2] = normalize(setup_time, 0.25, 0) * weights_new[machinenumber - 1][noAttributes + 2]
    attribute[3] = normalize(weight, 5, 1) * weights_new[machinenumber - 1][noAttributes + 3]  # Job Weight
...
def machine_processing(job_shop, current_WC, machine_number, env, weights_new):
    while True:
        ...
        if current_items:
            ...
            #                                 remain_processing_time(job), env, setup_time, job.weight)
...
def do_simulation_with_weights(mean_weight_new, std_weight_new, arrival_time_mean, due_date_k, iii):
    ...
    for wc in range(len(machinesPerWC)):
        for ii in range(machinesPerWC[wc]):
            env.process(machine_processing(job_shop, wc + 1, ii, env, test_weights_pos))
    job_shop.end_event = env.event()
    env.run(until=job_shop.end_event)
    if job_shop.early_termination == 1:
        objective_new[0] = np.mean(np.nonzero(job_shop.tardiness[499:2499])) + 10_000 - np.count_nonzero(job_shop.makespan[499:2499])
    else:
    ...
    for wc in range(len(machinesPerWC)):
        for ii in range(machinesPerWC[wc]):
            env.process(machine_processing(job_shop, wc + 1, ii, env, test_weights_min))
    job_shop.end_event = env.event()
    env.run(until=job_shop.end_event)
    if job_shop.early_termination == 1:
        objective_new[1] = np.mean(np.nonzero(job_shop.tardiness[499:2499])) + 10_000 - np.count_nonzero(job_shop.makespan[499:2499])
    else:
...
def run_linear(filename1, filename2, arrival_time_mean, due_date_k):
    ...
    for num_sim in range(no_generation):
        for ii in range(2):
            seeds = range(int(population_size / 2))
            ...
            for h, j in zip(range(population_size), seeds):
                objective[ii * int(population_size / 2) + j] = makespan_per_seed[h][0]
                eta[ii * int(population_size / 2) + j] = makespan_per_seed[h][1]
        ...
        alpha_mean = 0.1 * np.exp(-(t - 1) / 5000)
        alpha_std = 0.025 * np.exp(-(t - 1) / 5000)
        ...
        L = [str(num_sim) + " ", str(np.mean(np.mean(objective, axis=0))) + " ",
             str(np.mean(np.mean(np.exp(std_weight)))) + "\n"]
        file1.writelines(L)
...
if __name__ == '__main__':
    arrival_time = [1.5429, 1.4572, 1.4572, 1.3804, 1.3804, 1.3804]
    utilization = [85, 90, 90, 95, 95, 95]
    due_date_settings = [4, 4, 6, 4, 6, 8]
    for num_runs in range(len(utilization)):
        print("Current run is:" + str(utilization[num_runs]) + "-" + str(due_date_settings[num_runs]))
        str1 = "Runs/Run-FCFS-" + str(utilization[num_runs]) + "-" + str(due_date_settings[num_runs]) + ".txt"
        str2 = "Runs/Run-weights-FCFS-" + str(utilization[num_runs]) + "-" + str(due_date_settings[num_runs]) + ".csv"
        run_linear(str1, str2, arrival_time[num_runs], due_date_settings[num_runs])
