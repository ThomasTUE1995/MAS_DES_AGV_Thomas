...
def bid_calculation(weights_new, machinenumber, processing_time,
    ...
    print((((due_date - now) / total_rp) + 7) / (12 + 7))
...
def next_workstation(job, job_shop, env):
    if job.currentOperation + 1 <= job.numberOfOperations:
        ...
        # finished_job += 1
        if job.number > 2999:
            if np.count_nonzero(job_shop.makespan[999:2999]) == 2000:
...
def do_simulation_with_weights(mean_weight_new, arrivalMean, due_date_tightness, bid_skip, seq_skip, iter):
    ...
    ut = [job_shop.utilization[(jj, wc)] / job_shop.finish_time for wc in range(len(machinesPerWC)) for jj in
          range(machinesPerWC[wc])]
    print(ut)
    # print(job_shop.QueuesWC)
    #
    # if job_shop.early_termination == 1:
    #     objective = np.nanmean(np.nonzero(job_shop.tardiness[499:2499])) + 10_000 - np.count_nonzero(
    #         job_shop.makespan[499:2499])
    # else:
    #     objective = np.nanmean(job_shop.tardiness[499:2499])
    # # print(len(job_shop.tardiness))


    # df1 = pd.DataFrame(job_shop.QueuesWC[2])
    # fig, ax = plt.subplots(1)
    # WIP = [iter(len(job_shop.totalWIP)), job_shop.totalWIP]
    plt.plot(range(len(job_shop.totalWIP)), job_shop.totalWIP)
    # df1 = pd.DataFrame(WIP, index=[0])
    # axes1 = df1.plot()
    plt.show()
    #
    return objective
...
if __name__ == '__main__':
    ...
    final_result = np.zeros((1, len(skip_seq)))
    no_runs = 1
    for i, (skip_b, skip_s) in enumerate(zip(skip_bid, skip_seq)):
        str1 = "Runs/Attribute_Runs/90-4-1/Run-weights-NoSetup-90-4-5000" + "-" + str(skip_b) + "-" + str(
            skip_s) + ".csv"
        ...
        seeds = range(no_runs)
        func1 = partial(do_simulation_with_weights, weights, 1.4572, 4, skip_b, skip_s)
    ...
    filename2 = "Results/Attributes_New_95_8.csv"
