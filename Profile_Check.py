...
def bid_calculation_agv(bid_weights, agvnumber, normalization, agv, job, job_shop, processing_time,
    ...
    # TODO: Attribute 0 is very low with many AGVs
...
def agv_processing(job_shop, currentWC, agv_number, env, agv, normalization, agv_res, agv_buf,
                   agv_number_WC):
...
def do_simulation_with_weights(mean_weights, arrivalMean, due_date_tightness, min_job, max_job,
    ...
    mean_WIP, early_term = get_objectives(job_shop, min_job, max_job, job_shop.early_termination)  # Gather all results
...
def main():
    ...
    for (a, b, c, d) in itertools.product(simulation_parameter_1, simulation_parameter_2, simulation_parameter_3,
        ...
        str1 = "Runs/AGV/Arr_1/Final_Runs/Run-weights-" + str(utilization[0]) + "-" + str(
            due_date_settings[0]) + ".csv"

        df = pd.read_csv(str1, header=None)
        weights = df.values.tolist()

        print("Current run is: " + str(utilization[0]) + "-" + str(due_date_settings[0]))

        seed = 0

        do_simulation_with_weights(weights, arrival_time[0],
                                   due_date_settings[0],
                                   min_jobs[0], max_jobs[0], normaliziation_MA_array[0], normalization_AGV_array[0],
                                   wip_max[0], AGV_rule,
                                   created_travel_time_matrix, immediate_release_bool, JAFAMT_value, agvsPerWC_list,
                                   agv_number_WC_list, seed)
