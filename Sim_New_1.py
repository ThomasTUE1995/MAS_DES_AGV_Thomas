...
def bid_winner(env, job, noOfMachines, currentWC, job_shop, last_job, makespan_currentWC, machine, store, bid_skip,
    ...
    dueDateVector = np.zeros(no_of_jobs)
    setup_time_mean = np.zeros(noOfMachines)
    for ii in range(no_of_jobs):
    ...
    setup_time_mean = setup_time_mean / no_of_jobs
    meanProcessingTime = meanProcessingTime / no_of_jobs
    ...
    for jj in range(noOfMachines):
        ...
        for ii, j in enumerate(job):
            if len(machine[(jj, currentWC - 1)].items) > 0:
                # setup_time = expected_setup_time(j, job_shop, machine[jj].items)
                lastjob = machine[(jj, currentWC - 1)].items[len(machine[(jj, currentWC - 1)].items) - 1].type
                setup_time = setupTime[j.type - 1][lastjob - 1]
            else:
                setup_time = 0
            ...
            #                              env.now,
            #                              j.priority, queue_length, bid_skip, normalization)
    ...
    for dup in sorted_list:
        ...
        for ii in dup[1]:
            if (bestbid <= current_bid[ii]): # | (
                    #(bestbid == current_bid[ii]) & (len(machine[(ii, currentWC - 1)].items) < len(machine[(bestmachine, currentWC - 1)].items))):
...
def next_workstation(job, job_shop, env, min_job, max_job):
    if job.currentOperation + 1 <= job.numberOfOperations:
        ...
        #
        # if (job_shop.WIP > 300) | (env.now > 10_000):
        #     job_shop.end_event.succeed()
...
def machine_processing(job_shop, current_WC, machine_number, env, weights_new, relative_machine, last_job, machine,
                       makespan, seq_skip, utilization, min_job, max_job, normalization):
    while True:
...
def do_simulation_with_weights(mean_weight_new, arrivalMean, due_date_tightness, bid_skip, seq_skip, min_job, max_job,
    ...
    if job_shop.early_termination == 1:
        ...
        flow_time = np.nanmean(job_shop.makespan[min_job:max_job]) + 10_000 - np.count_nonzero(
            job_shop.makespan[min_job:max_job])
        ...
        for i in range(min_job, len(job_shop.tardiness)):
...
if __name__ == '__main__':
    ...
    for i in range(len(utilization)):
        ...
        for j in range(4):
            ...
            func1 = partial(do_simulation_with_weights, weights, arrival_time[i], due_date_settings[i], skip_bid[0],
                            skip_seq[0],
                            min_jobs[i], max_jobs[i], normaliziation[i])
            makespan_per_seed = jobshop_pool.map(func1, seeds)
            print(makespan_per_seed)
            for h, o in itertools.product(range(25), range(9)):
    ...
    results = pd.DataFrame(results,
                           columns=['Makespan', 'Mean Flow Time', 'Mean Weighted Tardiness', 'Max Weighted Tardiness',
                                    'No. Tardy Jobs P1', 'No. Tardy Jobs P2', 'No. Tardy Jobs P3', 'Mean WIP', 'Early_Term'])
    file_name = f"Results/Custom_2.csv"
