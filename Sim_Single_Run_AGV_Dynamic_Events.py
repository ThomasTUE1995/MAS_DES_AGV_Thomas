    ...
    for jj in range(noOfAGVs):
        if job_shop.agv_av[(jj, currentWC - 1)] == 0:
            ...
            for ii, job in enumerate(jobs):
                ...
                # TODO: Cancel bid if AGV is down
...
def next_workstation(job, job_shop, env, min_job, max_job, max_wip):
    ...
    if job.currentOperation + 1 <= job.numberOfOperations:
        ...
        # Let the AGV processing know that the destination is set
...
def get_objectives(job_shop, min_job, max_job, early_termination):
    ...
    for wc in range(len(machinesPerWC)):
        for ii in range(machinesPerWC[wc]):
            ...
            availability_rate_MA.append(1 - round(job_shop.ma_unavailability[(ii, wc)] / job_shop.finish_time, 2))

    # AGV utilizations
    utilization_AGVs = []
    availability_rate_AGV = []
    for wc in range(len(agvsPerWC)):
        for ii in range(agvsPerWC[wc]):
            ...
            availability_rate_AGV.append(1 - round(job_shop.agv_unavailability[(ii, wc)] / job_shop.finish_time, 2))

    return makespan, flow_time, mean_tardiness, max_tardiness, no_tardy_jobs_p1, no_tardy_jobs_p2, no_tardy_jobs_p3, \
           mean_WIP, early_term, utilization_MAs, utilization_AGVs, availability_rate_MA, availability_rate_AGV
...
def do_simulation_with_weights(mean_weights, arrivalMean, due_date_tightness, min_job, max_job,
                               ...
                               immediate_release, JAFAMT, agvsPerWC, agv_number_WC,
                               repair_time_ma, unav_prob_ma, repair_time_agv,
                               unav_prob_agv, iter1):
    ...
    for wc in range(len(machinesPerWC)):
        ...
        for ii in range(machinesPerWC[wc]):
            ...
            machines = Machine(job_shop, wc + 1, machine_number_WC[wc][ii], env, last_job,
                               ...
                               unav_prob_ma, repair_time_ma, machine_buf, JAFAMT)
        ...
        for ii in range(agvsPerWC[wc]):
            ...
            agvs = AGV(job_shop, wc + 1, agv_number_WC[wc][ii], env, agv, normalization_AGV, unav_prob_agv,
                       repair_time_agv, agv_buf, agv_number_WC)
    ...
    mean_WIP, early_term, utilization_result_MA, utilization_result_AGV, availability_rate_MA, \
    availability_rate_AGV = get_objectives(job_shop, min_job, max_job, job_shop.early_termination)  # Gather all results
...
class jobShop:
    ...
    def __init__(self, env, stored_weights, travel_time_matrix, agvsPerWC, agv_number_WC):
        ...
        self.ma_unavailability = {(ii, jj): 0 for jj in noOfWC for ii in range(machinesPerWC[jj])}
        self.agv_unavailability = {(ii, jj): 0 for jj in noOfWC for ii in range(agvsPerWC[jj])}
...
class AGV:
                ...
                if agv_location != next_job.location:
                    ...
                    while done_in:
                        try:
                            ...
                            repair_time = Random_Numpy.uniform(0, self.MTTA_mean * 2)
                            yield self.env.timeout(repair_time)
                            job_shop.agv_av[(relative_agv, currentWC - 1)] = 0
                            job_shop.agv_unavailability[(relative_agv, currentWC - 1)] = job_shop.agv_unavailability[(
                                relative_agv, currentWC - 1)] + repair_time
                            self.broken = False
                            debug(24, self.env, None, currentWC, None, agv_number, None, None)
    ...
    def break_agv(self, agv_number, currentWC):
        ...
        while True:
            ...
            yield self.env.timeout(Random_Numpy.expovariate(self.MTBA_mean))
            debug(20, self.env, None, currentWC, None, agv_number, None, None)
...
class Machine:
                ...
                if next_job not in machine_buf[0].items:
                    for attempt in range(999):
                        try:
                            ...
                            debug(23, self.env, None, currentWC, machine_number, None, None, None)
                            job_shop.ma_unavailability[(relative_machine, currentWC - 1)] = job_shop.ma_unavailability[(
                                relative_machine, currentWC - 1)] + repair_time
                            self.broken = False
    ...
    def break_machine(self, machine_number, currentWC):
        ...
        while True:
            ...
            yield self.env.timeout(Random_Numpy.expovariate(self.MTBA_mean))
            debug(19, self.env, None, currentWC, machine_number, None, None, None)
...
for (a, b, c, d, e) in itertools.product(simulation_parameter_1, simulation_parameter_2, simulation_parameter_3,
        ...
        if True:
            ...
            agv_availability_rate = []
            average_ma_utilization = []
            average_agv_utilization = []
            average_ma_availability_rate = []
            average_agv_availability_rate = []
            total_average_machine_utilization = []
            total_average_avg_utilization = []
            total_average_ma_availability_rate = []
            total_average_agv_availability_rate = []
        ...
        for i in range(len(utilization)):
            ...
            str1 = "Runs_Dynamic_Events/" + sim_par_1_string + "/" + sim_par_2_string + "/" + simulation_parameter_5[0] + "/" + str(
            ...
            for j in range(int(no_runs / no_processes)):
                ...
                func1 = partial(do_simulation_with_weights, weights, arrival_time[i],
                                ...
                                agv_number_WC_list, repair_time_ma[i], unav_prob_ma[i], repair_time_agv[i],
                                unav_prob_agv[i])
                ...
                for h, o in itertools.product(range(no_processes), range(9)):
                    ...
                    ma_availability_rate.append(makespan_per_seed[h][11])
                    agv_availability_rate.append(makespan_per_seed[h][12])
            ...
            average_ma_availability_rate.append(np.nanmean(ma_availability_rate, axis=0))
            average_agv_availability_rate.append(np.nanmean(agv_availability_rate, axis=0))

        # Print utilizations Machine Agents and AGV agents
        print("\n=============== Machine & AGV Utilization info ===============")
        for wc in range(len(machinesPerWC)):
            print("\n", " Work Center", wc)
            print("--------------------")
            for ma in machine_number_WC[wc]:
                ...
                total_average_ma_availability_rate.append(round(average_ma_availability_rate[0][ma - 1] * 100, 2))
            if simulation_parameter_1[0] == 1:
                for agv in agv_number_WC_new[wc]:
                    ...
                    total_average_agv_availability_rate.append(
                        round(average_agv_availability_rate[0][agv - 1] * 100, 2))
        ...
        results = pd.DataFrame(results, columns=['Makespan', 'Mean Flow Time', 'Mean Weighted Tardiness',
                                                 'Max Weighted Tardiness', 'No. Tardy Jobs P1',
                                                 'No. Tardy Jobs P2',
        ...
        print("Total Average Machine Availability:",
              round(sum(total_average_ma_availability_rate) / len(total_average_ma_availability_rate), 2), "%")
        print("Total Average Avg Availability:",
              round(sum(total_average_agv_availability_rate) / len(total_average_agv_availability_rate), 2), "%")
